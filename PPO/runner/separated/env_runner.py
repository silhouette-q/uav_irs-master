...
from PPO.runner.separated.base_runner import Runner
...
class EnvRunner(Runner):
    ...
    def eval(self, total_num_steps):
        ...
        origin_env = [env.env.proxy_env for env in self.eval_envs.envs]

        assert (eval_episode_rewards.shape[1] == self.n_eval_rollout_threads)
        eval_average_episode_rewards = np.sum(np.mean(eval_episode_rewards, axis=1))
        print("eval average episode rewards" + str(eval_average_episode_rewards))
        self.writter.add_scalar("eval/reward", np.array(eval_average_episode_rewards), self.episode)
