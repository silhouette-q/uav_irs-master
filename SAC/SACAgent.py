...
class SACAgent(Base_Agent):
    ...
    def __init__(self, config, agent_id, state_size, action_size, agent_num=8):
        ...
        self.critic_local = self.create_NN(input_dim=self.state_size + (self.action_size) * agent_num, output_dim=1,
                                           key_to_use="Critic")
        self.critic_local_2 = self.create_NN(input_dim=self.state_size + (self.action_size) * agent_num, output_dim=1,
        ...
        self.critic_target = self.create_NN(input_dim=self.state_size + (self.action_size) * agent_num, output_dim=1,
                                            key_to_use="Critic")
        self.critic_target_2 = self.create_NN(input_dim=self.state_size + (self.action_size) * agent_num, output_dim=1,
                                              key_to_use="Critic")
    ...
    def pick_action(self, eval_ep, state=None):
        ...
        if eval_ep:
            ...
            action = torch.from_numpy(self.action_space.sample()).float().unsqueeze(0).to(self.device)
        else:
            action = self.actor_pick_action(state=state[self.agent_id, :])
        ...
        if not eval_ep:
            ...
            if self.add_extra_noise:
                # 确保噪声与action在相同设备和类型
                noise_np = self.noise.sample()
                noise_tensor = torch.from_numpy(noise_np).float().to(self.device)
                action += noise_tensor
            if self.config.add_gravity_noise:
                if np.random.random(1) > self.parent.episode_number / self.config.num_episodes_to_run:
                    # 确保状态数据在正确设备上
                    if isinstance(state, torch.Tensor):
                        state_input = state[self.agent_id, :].unsqueeze(0)
                    else:
                        state_input = torch.from_numpy(state[self.agent_id, :]).float().unsqueeze(0).to(self.device)
                    action = add_gravity_exploration_noise(state_input, action)
        return action
    ...
    def actor_pick_action(self, state=None, eval=False):
        ...
        # --- 开始修改 ---
        # 如果 state 是 numpy 数组, 将其转换为 torch 张量并移动到正确的设备
    ...
    def calculate_critic_losses(self, state_batch, action_batch, reward_batch, next_state_batch, mask_batch):
        ...
        with torch.no_grad():
            ...
            #     min_qf_next_target)
            # --- 开始修改 ---
            # 确保 reward_batch 具有 [256, 1] 的形状以避免广播错误
            reward_for_agent = reward_batch[:, self.agent_id].unsqueeze(1)
            next_q_value = reward_for_agent + self.hyperparameters["discount_rate"] * (
                min_qf_next_target)
            # --- 修改结束 ---

        critic_input_batch = torch.cat(
            [state_batch[:,self.agent_id,:].reshape(state_batch.shape[0], -1),
             action_batch.reshape(state_batch.shape[0], -1)], axis=1)
    ...
    def calculate_actor_loss(self, state_batch, action_batch):
        ...
        critic_input_batch = torch.cat(
            [state_batch[:,self.agent_id,:].reshape(state_batch.shape[0], -1),
             actions_input.reshape(actions_input.shape[0], -1)], axis=1)
